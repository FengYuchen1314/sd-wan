...
主动加入的后续设备会先询问“是否有可供其他节点主动拨入的公网入口”。选择无公网时，只需选择本机面板端口并设置密码；控制中继和 WireGuard 本地端口都会静默选择空闲值，不会发布成固定 Endpoint。该节点只主动拨号有公网入口的父节点，也不能与另一台无公网节点建立直接连接。选择有公网时，安装器才会继续询问公网 IP/域名、控制中继端口和 WireGuard UDP 端口。
...
面板总览和“配置发布”页还提供“一键更新全网”：所有在线节点先并行探测 GitHub，首个成功节点把安装器和 gzip 制品连同 SHA-256 摘要交回控制面，控制面再沿现有命令通路把同一份制品送到每台设备。其他设备不需要直连 GitHub 或更新源节点；每台设备的排队、应用和失败状态会显示在面板。安装器会部署本机 root 级 systemd 更新调度器，Web/Agent 只能提交经过摘要校验的暂存请求，更新完成后仍执行服务健康检查与自动回滚。
...
- NAT 被动侧不使用节点自报端口，只从 WireGuard 认证握手动态学习真实出口 Endpoint
- 普通拓扑链路握手故障时自动绕开，保留无路由健康 Peer 继续复检，恢复后重新启用
- 全网可达性与地址冲突校验
- 基于拓扑的逐节点 WireGuard 配置编译
- 一次性加入令牌与安装命令生成
- 主动加入令牌分别绑定父节点控制入口与 WireGuard UDP 入口，NAT 子节点不会因自报端口不可达而注册回滚
- 节点显式记录公网拨入能力；无公网节点不发布 Endpoint、只能拨号公网节点，两个无公网节点禁止直连
- 面板一键更新全网；自动选择可访问 GitHub 的节点并通过控制面分发同一 SHA-256 制品
